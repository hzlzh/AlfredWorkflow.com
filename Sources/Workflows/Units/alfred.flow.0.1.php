...
class Alfredflow {
	...
	private $cache; 		// path to..
	private $data;			// path to..
	private $bundle; 		// bundleid
	private $path; 			// folder
	private $home; 			// homefolder
	private $results; 	// placeholder for results
	...
	function __construct( $bundleid =null )
	{
		...
		if ( file_exists( 'info.plist' ) ):
			$this->bundle = $this->get( 'bundleid', 'info.plist' ); //try to get bundleid from info.plist
		endif;

		if ( !is_null( $bundleid ) ):
			$this->bundle = $bundleid; // or use bundle is from call
		endif;
